...
const (
	defaultServicesURL = "https://api.byteplus.com/api/common/explorer/services"
	defaultApisURL     = "https://api.byteplus.com/api/common/explorer/apis"
)
...
func main() {
	...
	metadataDir := flag.String("metadata-dir", filepath.Join(root, "byteplus-sdk-metadata", "metadata"), "metadata directory")
	assetGo := flag.String("asset-go", filepath.Join(root, "asset", "asset.go"), "generated asset.go fallback")
	out := flag.String("out", filepath.Join(root, "byteplus-sdk-metadata", "explorer_descriptions", "descriptions.json"), "output JSON path")
	flag.Parse()
...
func loadVersionsFromBindata(assetGo string) map[string][]string {
	...
	pattern := regexp.MustCompile(`byteplus-sdk-metadata/metadata/([^/]+)/([^/]+)/metadata\.json`)

...
func TestParserReadsEqualsFormFlags(t *testing.T) {
	ctx := NewContext()
	parser := NewParser([]string{
		"---profile=release",
		"---region=ap-southeast-1",
	...
	if got := ctx.dynamicFlags.GetByName("Filter").GetValue(); got != "a=b" {
		t.Fatalf("dynamic flag Filter = %q, want a=b", got)
	}
}

func TestParserRejectsEmptyEqualsValue(t *testing.T) {
	ctx := NewContext()

...
const (
	envCLIDebug        = "BYTEPLUS_CLI_DEBUG"
	envCLIDebugLogFile = "BYTEPLUS_CLI_DEBUG_LOG_FILE"
...
// openDebugLogFile 会在打开前后校验目标文件，避免 debug 内容被追加到 symlink/hardlink 指向的非预期文件。
...
func verifyOpenedDebugLogFile(path string, file *os.File) error {
	...
	return validateDebugLogFileInfo(path, fileInfo, file)
}
...
func isSensitiveDebugKey(key string) bool {
	...
	// ak/sk/pwd/sign 等短词只做精确匹配，避免误伤 TaskId、DiskId 这类普通业务字段。

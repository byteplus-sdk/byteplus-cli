...
```powershell
bp completion powershell > bp.ps1
```
...
```shell
BYTEPLUS_CLI_DEBUG=false
BYTEPLUS_CLI_DEBUG=0
BYTEPLUS_CLI_DEBUG=off
BYTEPLUS_CLI_DEBUG=no
BYTEPLUS_CLI_DEBUG=
```
...
```text
~/.byteplus/logs/YYYYMMDDHH.log
```

Example:

```text
~/.byteplus/logs/2026061814.log
```
...
`bp configure sso` writes an SSO profile but does not switch current. Run:
...
bp sso login --sso-session my-sso --no-browser
```
